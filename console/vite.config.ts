...
import { runtimeAssetReplacements } from "./runtimeAssetReplacements";

function replaceRuntimeAssetUrls() {
  return {
    name: "replace-runtime-asset-urls",
    ...
    transform(code: string, id: string) {
      ...
      for (const [remoteUrl, localPath] of Object.entries(
        runtimeAssetReplacements,
      )) {
        nextCode = nextCode.split(remoteUrl).join(localPath);
      }
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [replaceRuntimeAssetUrls(), react()],
    css: {
      modules: {
